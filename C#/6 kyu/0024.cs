...
// Charlie doesn't like these sessions, since the motif usually repeats. He isn't 
...
// For example if the input number is 2, and the input list is [1,2,3,1,2,1,2,3], you 
// take [1,2,3,1,2], drop the next [1,2] since this would lead to 1 and 2 being in the 
...
public class Kata {
  public static int[] DeleteNth(int[] arr, int x) {
    ...
    for (var i = 0; i < arr.Length; ++i) {
      ...
      if (dict[arr[i]] <= x) passed.Add(arr[i]);
    }
