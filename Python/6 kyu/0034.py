...
# Link: https://www.codewars.com/kata/5a48948e145c46820b00002f
...
# The k-prime numbers are not regularly spaced. For example: between 2 and 50 we
# have the following 2-primes:
...
#   - step (integer > 0) which indicates the step we want to find between two
#     k-primes,
#   - start (integer >= 0) which gives the start of the search (start
#     inclusive),
...
# So this function should return an array of all the pairs (or tuples) of
# k-prime numbers spaced with a step of step between the limits start, nd. This
# array can be empty.
...
def kprimes_step(k, step, start, nd):
    res = []
    for i in range(start, nd - step + 1, 1):
        k_prime = has_k_prime_factors(i, k)
        if k_prime:
            if has_k_prime_factors(i + step, k):
                res.append([i, i + step])
    return res


def has_k_prime_factors(n, k):
    count = 0
    i = 2
    root = math.sqrt(n)
    while i <= n and i <= root:
        if n % i == 0:
            n /= i
            count += 1
        else:
