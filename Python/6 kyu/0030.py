...
# x. The formula to compute n choose x is:
#   n! / (x! * (n - x)!)
...
# a different combination of 3 colors or 4 colors. You will take 3 colors
# because it's less expensive.

# Hence the problem:
# Knowing m (number of posters to design) and n (total number of available
# colors), let us search x, the number of colors for each poster so that each
# poster has a unique combination of colors and the number of combinations is
# exactly the same as the number of posters m.

# In other words, you must find x such that:
#  - n choose x = m
# for a given m >= 0 and a given n > 0. If there are several solutions, return
