...
def score(dice):
    res = 0
    cnt = Counter(dice)
    res += add_score(1, 3, 1000, cnt)
    res += add_score(6, 3, 600, cnt)
    res += add_score(5, 3, 500, cnt)
    res += add_score(4, 3, 400, cnt)
    res += add_score(3, 3, 300, cnt)
    res += add_score(2, 3, 200, cnt)
    res += add_score(1, 1, 100, cnt)
    res += add_score(5, 1, 50, cnt)
    return res


def add_score(dice: int, amount: int, points: int, cnt: dict) -> int:
    if cnt[dice] >= amount:
        times = cnt[dice] // amount
        cnt[dice] -= amount * times
        return points * times
    return 0
