...
function queueTime(customers, n) {
  let checkouts = new Array(n);
  for (let i = 0; i < checkouts.length; ++i) checkouts[i] = 0;
  customers.forEach((customer) => {
    let nextCheckout = 0;
    for (let j = 1; j < checkouts.length; ++j) {
      if (checkouts[j] < checkouts[nextCheckout]) nextCheckout = j;
    }
    checkouts[nextCheckout] += customer;
  });
