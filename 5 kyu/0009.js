// Kata: Help your granny!
// Link: https://www.codewars.com/kata/5536a85b6ed4ee5a78000035
...
// Image address: http://i.imgur.com/dG7iWXhm.jpg
...
function tour(friends, fTowns, distTable) {
  ...
  for (let i = 0; i < distTable.length; i += 2) {
    distance[distTable[i]] = distTable[i + 1];
    orderOfDistance.push(distTable[i]);
  }

  //Pythagorean theorem
  const calcDistance = (sideIndex, hypotenuseIndex) =>
    Math.sqrt(distance[orderOfDistance[hypotenuseIndex]] ** 2 - distance[orderOfDistance[sideIndex]] ** 2);

  // create a new array of the town names in the order they get visited
  friends.forEach((friend) =>
    fTowns.filter((fTown) => fTown[0] === friend).forEach((fTown) => orderOfVisit.push(fTown[1]))
  );

  // reaching first town
  result += distance[orderOfVisit[0]];

  // go through the town names in the order they get visited
  for (let i = 1; i < orderOfVisit.length; ++i) {
    let smallIndex, largeIndex;
    orderOfDistance.forEach((town, index) => {
      if (town === orderOfVisit[i - 1]) smallIndex = index;
      if (town === orderOfVisit[i]) largeIndex = index;
    });

    // if we are going backwards, switch indexes to calculate distance
    // the length of the hypotenuse always belongs to the town with the larger index
    if (smallIndex > largeIndex) [smallIndex, largeIndex] = [largeIndex, smallIndex];
    result += calcDistance(smallIndex, largeIndex);
  }

  //Going back home
  result += distance[orderOfVisit[orderOfVisit.length - 1]];
